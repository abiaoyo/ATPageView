...
@implementation DDOCScrollPageBarStyleModel
- (id)init{
    if(self = [super init]){
        ...
        self.normalColor = DDOCScrollPAGE_COLOR_HEXA(0xC6C6C6, 1.0);
        self.selectedColor = DDOCScrollPAGE_COLOR_HEXA(0x494949, 1.0);
    }
    return self;
}
@end

@interface DDOCScrollPageBarItemModel : NSObject
...
@property (nonatomic,strong) DDOCScrollPageBarStyleModel * styleModel;

- (id)initWithTitle:(NSString *)title itemMaxSize:(CGSize)itemMaxSize contentX:(CGFloat)contentX styleModel:(DDOCScrollPageBarStyleModel *)styleModel;

@end

@implementation DDOCScrollPageBarItemModel
...
- (void)createLayout{
    ...
    DDOCScrollPageBarStyleModel * styleModel = self.styleModel;
    
    if(styleModel.style == DDOCScrollPageBarDefault){
        cursorWidth = MAX(styleModel.progressWidth, size.width-20);
    }else if (styleModel.style == DDOCScrollPageBarFixed){
        cursorWidth = styleModel.progressWidth;
    }else if (styleModel.style == DDOCScrollPageBarReptile){
        cursorWidth = styleModel.progressWidth;
    }
...
@interface DDOCScrollPageBarItemView : UIView
@property (nonatomic,strong) UILabel * titleLabel;
@end

@implementation DDOCScrollPageBarItemView
...
#define DDOCScrollPageBarViewItemTagS 100

@interface DDOCScrollPageBarView()
@property (nonatomic,strong) UIView * progressView;
@property (nonatomic,strong) UIScrollView * scrollView;
@property (nonatomic,strong) NSMutableArray<DDOCScrollPageBarItemModel *> * itemArray;
@property (nonatomic,strong) DDOCScrollPageBarStyleModel * styleModel;
@property (nonatomic,weak) DDOCScrollPageBarItemView * selectedItemView;
@property (nonatomic,assign) CGFloat progressOffset;

@end


@implementation DDOCScrollPageBarView
...
- (void)clickScrollPageItemView:(UITapGestureRecognizer *)tap{
    ...
    NSInteger fromIndex = self.selectedItemView.tag-DDOCScrollPageBarViewItemTagS;
    NSInteger toIndex = tap.view.tag-DDOCScrollPageBarViewItemTagS;

    DDOCScrollPageBarItemModel * toItemModel = self.itemArray[toIndex];
    DDOCScrollPageBarItemView * fromItemView = [self.scrollView viewWithTag:(DDOCScrollPageBarViewItemTagS+fromIndex)];
    DDOCScrollPageBarItemView * toItemView = [self.scrollView viewWithTag:(DDOCScrollPageBarViewItemTagS+toIndex)];
    ...
    if(self.delegate && [self.delegate respondsToSelector:@selector(pageBarView:selectedIndex:)]){
        [self.delegate pageBarView:self selectedIndex:tap.view.tag-DDOCScrollPageBarViewItemTagS];
    }
}

#pragma mark - DDOCScrollPageBarViewProtocol
...
- (NSInteger)selectedItemIndex{
    ...
    if(self.dataSource && [self.dataSource respondsToSelector:@selector(selectedItemIndexInPageBarView:)]){
        selectedItemIndex = [self.dataSource selectedItemIndexInPageBarView:self];
    }
...
- (NSArray<NSString *> *)titles{
    if(self.dataSource && [self.dataSource respondsToSelector:@selector(titlesInPageBarView:)]){
        return [self.dataSource titlesInPageBarView:self];
    }
    return nil;
}
...
- (void)reloadData{
    ...
    if(titles){
        CGFloat contentX = 0;
        DDOCScrollPageBarItemModel * defaultItemModel = nil;
        DDOCScrollPageBarItemView * defaultItemView = nil;
        ...
        for(NSInteger i=0;i<itemCount;i++){
            ...
            DDOCScrollPageBarItemModel * itemModel = [[DDOCScrollPageBarItemModel alloc] initWithTitle:title itemMaxSize:CGSizeMake(1000, selfHeight) contentX:contentX styleModel:_styleModel];
            [itemModel createLayout];
            [self.itemArray addObject:itemModel];
            
            DDOCScrollPageBarItemView * itemView = [[DDOCScrollPageBarItemView alloc] initWithFrame:CGRectZero];
            itemView.titleLabel.attributedText = itemModel.titleAttributedText;
            itemView.frame = itemModel.itemFrame;
            itemView.tag = DDOCScrollPageBarViewItemTagS+i;
...
//        v.contentInset = self.styleModel.contentInset;
...
- (CGPoint)convertProgressCenter:(DDOCScrollPageBarItemModel *)fromItemModel toItemModel:(DDOCScrollPageBarItemModel *)toItemModel progress:(float)progress{
...
- (void)handleBarStyleDefaultContainerDidScroll:(UIScrollView *)scrollView
    ...
    DDOCScrollPageBarItemModel * fromItemModel = self.itemArray[fromIndex];
    DDOCScrollPageBarItemModel * toItemModel = self.itemArray[toIndex];
